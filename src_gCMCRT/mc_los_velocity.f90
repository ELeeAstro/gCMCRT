...
contains
  ...
  subroutine compute_vlos(viewphi)
    implicit none

    integer :: i,j,k,n
    real(dp) :: u, v, w
    real(dp), dimension(n_phase), intent(in) :: viewphi
    ...
    allocate(v_los(n_phase,grid%n_lay,grid%n_phi-1,grid%n_theta-1))
