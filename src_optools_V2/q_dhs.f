...
c maxf	f_max of the DHS (between 0 and 1).
	subroutine q_dhs(e1,e2,lam,rad,cext,csca,g,maxf)
	IMPLICIT NONE
	real*8 e1,e2,lam,rad,pi,maxf,g
	real*8 tot,cext,csca,cabs
	integer n,i,err
	real*8 f(50),wf(50)
	complex min,m
	real rad0,qe1,qa1,qs1,wvno,mu(10),qbs,gqsc
	real m1(10,2),m2(10,2),d21(10,2),s21(10,2),rcore
	mu(1)=0.1
	mu(2)=0.4
	cext=0.
	csca=0.
	...
	tot=0.
c============================================================================
c number of vacuum fractions used
c please check if this is sufficient! might not be for large values of f_max
	n=20
c============================================================================
	call gauleg(0d0,maxf,f,wf,n)
	do i=1,n
		tot=tot+wf(i)
	enddo
	g=0d0
	do i=1,n
		...
		wvno=2d0*pi/lam
		rad0=rad/((1.-f(i))**(1./3.))
		rcore=rad0*f(i)**(1d0/3d0)
		err=0
		call DMiLay(RCORE, rad0, WVNO, m, min, MU,
     &                   2, QE1, QS1, QBS, GQSC, 
     &                   M1, M2, S21, D21, 10 ,err)
		qa1=qe1-qs1
		if(qe1.le.0d0) qe1=0d0
		if(qs1.le.0d0) qs1=0d0
		if(qa1.le.0d0) qa1=0d0
		cext=cext+pi*rad0**2*wf(i)*qe1/tot
		csca=csca+pi*rad0**2*wf(i)*qs1/tot
		cabs=cabs+pi*rad0**2*wf(i)*qa1/tot
