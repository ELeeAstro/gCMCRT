...
c     .. Scalar Arguments ..
      ...
      double precision      GQSC, QBS, QEXT, QSCA, RCORE, RSHELL, WVNO
      double COMPLEX   RINDCO, RINDSH
c     ..
c     .. Array Arguments ..

      double precision MU( NUMANG ), D21( MAXANG, 2 ), M1( MAXANG, 2 ),
...
c                        ** Check input arguments for gross errors
     ...
     &    INPERR = WRTBAD( 'Rcore' )
      ...
      IF( REAL(RINDSH).LE.0.0 .OR. AIMAG(RINDSH).GT.0.0 ) then
